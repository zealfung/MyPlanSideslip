...
- (void)event {
    ...
    self.lockView.setPWTwiceSameBlock = ^(NSString *pwd){
        ...
        if(CoreLockTypeModifyPwd == _type){
            [self.label showNormalMsg:CoreLockModifySuccessTitle];
        } else {
...
//        NSString *pwdLocal = [CoreArchive strForKey:CoreLockPWDKey];
        NSString *pwdLocal = [Config shareInstance].settings.gesturePasswod;
        ...
        if(res){//密码一致
            ...
            if(CoreLockTypeVeryfiPwd == _type || CoreLockTypeLogInVeryfiPwd == _type){
                ...
                [self.label showNormalMsg:CoreLockModifyNewPWDTitle];
...
- (void)vcPrepare {
    ...
    if (CoreLockTypeLogInVeryfiPwd == _type) {
...
//    return pwd !=nil;
    ...
    NSString *pwd = [Config shareInstance].settings.gesturePasswod;
...
+ (instancetype)showVerifyLockVCInVC:(UIViewController *)vc isLogIn:(BOOL)isLogIn forgetPwdBlock:(void(^)())forgetPwdBlock successBlock:(void(^)(CLLockVC *lockVC, NSString *pwd))successBlock {
    ...
    if (isLogIn) {
        //登录类型
        lockVC.type = CoreLockTypeLogInVeryfiPwd;
    } else {
        //设置类型
